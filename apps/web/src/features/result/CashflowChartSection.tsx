    ...
    () =>
      result.map((r) => {
        ...
        return {
          ...
          year: r.year,
          age: r.age,
          手取り給与: Math.trunc(r.income.net),
          年金: Math.trunc(r.income.pension),
          児童手当: Math.trunc(r.income.childAllowance),
          その他収入: Math.trunc(r.income.other),
          住居費: -Math.trunc(e.housing),
          生活費: -Math.trunc(e.living),
          教育費: -Math.trunc(e.education),
          保険料: -Math.trunc(e.insurance),
          固定費: -Math.trunc(e.fixed),
          イベント費: -Math.trunc(e.events),
          balance: Math.trunc(r.balance),
          totalIncome: Math.trunc(totalIncome),
          totalExpense: Math.trunc(totalExpense),
