...
- チケット = GitHub Issue(本文に「依存: #N」を記載)
...
本文は `implement-issue` skill が読み取る形式に合わせる:
...
## 技術方針
<SPEC.mdの該当節、使用ライブラリ、既存コードとの整合など>
```
...
Readyなチケットそれぞれに対し、サブエージェント(general-purpose)を**同一ターンで並列に**起動する。同時実行は**3体まで**を目安にする(それ以上はレビュー待ちPRが積み上がるだけで、ユーザーのレビュー帯域を超える)。
